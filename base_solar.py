...
Based on huawei_solar from Emilv2
Modified to use serial RTU connection

ccorderor
https://github.com/ccorderor
"""
import asyncio
import logging
import time
...
class HuaweiSolar(_HuaweiSolarBase):
    ...
    def __init__(self, port, timeout=1, wait=1, slave=1):
        super().__init__(timeout=timeout, wait=wait, slave=slave)
        self.client = ModbusClient(method='rtu', port=port, timeout=1, baudrate=9600, stopbits = 1, bytesize = 8, parity = 'N')
        #self.client = ModbusClient(port='/dev/ttyUSB0', timeout=5, framer=ModbusRtuFramer)
